//
//  SearchMovieByNameYearAdapter.swift
//  ModeloMVC
//
...
class SearchMovieByNameYearAdapter: NSObject, SearchMovieAdapterProtocol {
...
extension SearchMovieByNameYearAdapter: UISearchBarDelegate {
    ...
    func searchBar(_ searchBar: UISearchBar, textDidChange searchText: String) {
        ...
        if searchTextLowercased.isEmpty {
            ...
            arrayResult = self.datasource.filter({
                $0.original_title.lowercased().contains(searchTextLowercased) || $0.release_date.contains(searchTextLowercased)
            })
